...
for(i = 0; i < tracks.length; i++){
  const claveGenre = (tracks[i].genre); //Obtenemos el genero
  if(!listaCanciones[claveGenre]){
    listaCanciones[claveGenre] = [] //Si no existe, creamos uno.
  }
  listaCanciones[claveGenre].push(tracks[i].title)
}
